...
(defn calc-region-indent-helper!
  ...
  # XXX
  (eprintf "last line index: %p" last-line-index)
  ...
  (def input-region
    (string/join input-lines "\n"))
  # XXX
  (eprintf "input region: %p" input-region)
  (def indented-region
    (fmt/format input-region))
  # XXX
  (eprintf "output region: %p" indented-region)
  #
  (def in-lines
    (string/split "\n" input-region))
  # XXX
  ...
  (for i 0 (length in-lines)
    (eprintf "%d: %p" i (get in-lines i)))
  (def out-lines
    (string/split "\n" indented-region))
  # XXX
  (eprint "out-lines")
  (for i 0 (length out-lines)
    (eprintf "%d: %p" i (get out-lines i)))
  # XXX
  (eprintf "total lines: %p" (length in-lines))
  (def diffs @[])
...
(defn calc-region-indent
  ...
  # XXX
  (eprintf "original region: %p" fragment)
  (def input-lines
    (string/split "\n" fragment))
...
(defn main
  ...
  (def indents
    (calc-region-indent (file/read stdin :all)
                        (tracev outer-offset)
                        (tracev start-offset)
                        (tracev n-inner-lines)))
  # working with emacs is easier this way
  (prin "(")
  (each [line indentation] (tracev indents)
